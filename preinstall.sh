...
vips_version_minimum=8.13.3
vips_version_latest_major_minor=8.13
...
install_libvips_from_source() {
  ...
  tar xzvf vips-$vips_version_full.tar.gz
...
if [ $enable_openslide -eq 1 ] && [ -z $vips_with_openslide ] && [ $openslide_exists -eq 0 ]; then
  if [ -f /etc/debian_version ]; then
    ...
    case "$DISTRO" in
      bionic|focal|jammy|jessie|vivid|wily|xenial|stretch|loki)
